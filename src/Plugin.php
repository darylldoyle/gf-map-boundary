...
/**
 * Core plugin bootstrap and integration with Gravity Forms hooks/filters.
 *
 * Why: Keeps hook registration, asset enqueues, and entry processing centralized
 * so behavior is predictable and easy to reason about for PHPStan and maintainers.
 */
class Plugin {
	/**
	 * Initialize hooks for loading the Add-On, field type, assets, and filters.
	 *
	 * @return void
	 */
	...
	/**
	 * Ensure GF Add-On framework is included and our AddOn instantiated.
	 * Why: Access to plugin settings via GF UI and lifecycle.
	 *
	 * @return void
	 */
	...
	/**
	 * Add our field button to the GF editor advanced fields group.
	 *
	 * @param array<int, array{name?:string,fields?:array<int, array<string,mixed>>}> $groups
	 *
	...
	/**
	 * Enqueue frontend assets conditionally only on forms that include this field.
	 * Why: Avoid loading Google Maps and assets globally to reduce page weight.
	 *
	 * @param array $form    GF form array for the page request.
	 * @param bool  $is_ajax Whether the form is loaded via AJAX.
	 *
	 * @return void
	 */
	public static function enqueueFrontend( $form, $is_ajax ): void {
		...
		wp_register_script( 'gfmb-frontend', GFMB_PLUGIN_URL . 'assets/js/gfmb-frontend.js', $deps, GFMB_VERSION,
			true );
		wp_localize_script( 'gfmb-frontend', 'GFMB_Config', [
			...
			'i18n'          => [
				...
				'noApiKey'      => esc_html__( 'Google Maps API key is not configured. Contact site admin.',
					'gf-map-boundary' ),
			],
		] );
	...
	/**
	 * Check whether the provided form contains our custom field type.
	 *
	...
	/**
	 * Retrieve Google Maps API key from GF AddOn settings or WP option fallback.
	 *
	 * @return string Non-empty trimmed key or empty string when not configured.
	 */
	public static function getApiKey(): string {
		$api_key = '';
		if ( class_exists( 'GFAddOn' ) && class_exists( AddOn::class ) && method_exists( AddOn::class,
				'get_instance' ) ) {
	...
	/**
	 * After an entry is saved, generate a static map image for each map_boundary field.
	 *
	 * Why: Defers external API calls and file IO to post-save to keep submission fast
	 * and reliable. Also ensures we only store a final image URL after successful save.
	 *
	 * @param array $entry Gravity Forms entry array; will be updated with image URL in field value.
	 * @param array $form  Gravity Forms form array.
	 *
	...
	public static function handleEntryPostSave( array $entry, $form ): array {
		...
		foreach ( $form['fields'] as $field ) {
			...
			$static_url = 'https://maps.googleapis.com/maps/api/staticmap?' . http_build_query( $query, '', '&',
					PHP_QUERY_RFC3986 );
			...
			$filename   = sprintf( 'gfmb-form%s-entry%s-field%s-%s.png', $form['id'], $entry['id'], $field_id,
			...
			if ( $save_center_lat !== '' && $save_center_lng !== '' ) {
				...
				$gmaps_link = sprintf( 'https://www.google.com/maps/@%s,%s,%sz',
					rawurlencode( (string) $save_center_lat ), rawurlencode( (string) $save_center_lng ),
	...
	/**
	 * Register the meta keys we use for center/zoom/Google Maps link so they are visible in GF UI.
	 *
	 * @param array<string, array<string, mixed>> $entry_meta Existing meta registry.
	...
	public static function registerEntryMeta( array $entry_meta, $form_id ): array {
		...
		foreach ( $form['fields'] as $field ) {
			...
			$field_label                                  = is_string( $field->label ) && $field->label !== '' ? $field->label : sprintf( __( 'Field %d',
				'gf-map-boundary' ), (int) $field->id );
			$fid                                          = (string) $field->id;
	...
	/**
	 * Filter merge tag output so the field renders a preview + extra details.
	 *
	 * @param list<mixed> $args Gravity Forms forwards a long, evolving list of args;
	 *                          we keep this variadic and defensively typed to remain compatible.
	 *
	 * @return mixed The filtered value for the merge tag.
	 */
	public static function mergeTagFilter( ...$args ) {
		...
		if ( $field instanceof GF_Field && $field->type === 'map_boundary' ) {
			...
			if ( $format === 'html' && $url && filter_var( $url, FILTER_VALIDATE_URL ) ) {
				$img_html = sprintf( '<img src="%s" alt="%s" style="max-width:100%%;height:auto;border:1px solid #e5e5e5;" />',
					esc_url( $url ), esc_attr( $field->label ) );
				$details  = '';
				if ( $center_lat !== '' && $center_lng !== '' ) {
					$lat_disp  = is_numeric( $center_lat ) ? number_format( (float) $center_lat, 6, '.',
						'' ) : esc_html( (string) $center_lat );
					$lng_disp  = is_numeric( $center_lng ) ? number_format( (float) $center_lng, 6, '.',
						'' ) : esc_html( (string) $center_lng );
					$zoom_disp = $zoom !== '' ? sprintf( ' (z%s)', esc_html( (string) $zoom ) ) : '';
					$details   .= sprintf( '<div style="margin-top:6px;">%s %s, %s%s</div>',
						esc_html__( 'Center:', 'gf-map-boundary' ), esc_html( $lat_disp ), esc_html( $lng_disp ),
						$zoom_disp );
				}
				if ( is_string( $gmaps_link ) && $gmaps_link !== '' ) {
					$details .= sprintf( '<div style="margin-top:4px;"><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></div>',
						esc_url( $gmaps_link ), esc_html__( 'Open in Google Maps', 'gf-map-boundary' ) );
				}
			...
			if ( $format !== 'html' ) {
				...
				if ( $center_lat !== '' && $center_lng !== '' ) {
					...
					if ( $zoom !== '' ) {
						$center_txt .= sprintf( ' (z%s)', (string) $zoom );
					}
					$parts[] = $center_txt;
				}
				if ( is_string( $gmaps_link ) && $gmaps_link !== '' ) {
	...
	/**
	 * Append human-friendly details to the admin field value display.
	 *
	 * @param list<mixed> $args Gravity Forms forwards value, field, entry, form.
	 *
	 * @return mixed The possibly-augmented value.
	 */
	public static function adminEntryFieldValue( ...$args ) {
		...
		if ( $center_lat !== '' && $center_lng !== '' ) {
			...
			$details  .= sprintf(
				...
				esc_html__( 'Center:', 'gf-map-boundary' ),
				esc_html( $lat_disp ),
				esc_html( $lng_disp )
			);
		}
		if ( is_string( $gmaps_link ) && $gmaps_link !== '' ) {
			$details .= sprintf(
				'<div style="margin-top:4px;"><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></div>',
				esc_url( $gmaps_link ),
				esc_html__( 'Open in Google Maps', 'gf-map-boundary' )
			);
		}
