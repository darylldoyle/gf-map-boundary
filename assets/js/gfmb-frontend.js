(
    function() {
        ...
        function initAll(context) {
            var containers = (
                context || document
            ).querySelectorAll('.gfmb-field');
            containers.forEach(function(container) {
                if (container.dataset.gfmbInitialized === '1') {
                    return;
                }
        ...
        function initField(container) {
            ...
            var drawingControlsEl = container.querySelector(
                '.gfmb-drawing-controls');
            ...
            if (!cfg.apiKeyPresent || typeof google === 'undefined' ||
                !google.maps) {
                // Disable controls if API not available
                [postcodeEl, locateBtn, undoBtn, clearBtn].forEach(
                    function(el) {
                        if (el) {
                            el.disabled = true;
                        }
                ...
                if (mapEl) {
                    mapEl.innerHTML = '<div class="gfmb-warning">' + (
                        cfg.i18n && cfg.i18n.noApiKey
                            ? cfg.i18n.noApiKey
            ...
            function saveViewportToHidden() {
                if (!centerLatEl || !centerLngEl || !zoomEl) {
                    return;
                }
                var c = map.getCenter();
                if (c) {
            ...
            function setLocatedState(located) {
                if (located) {
                    ...
                    mapEl.style.display = '';
                    if (drawingControlsEl) {
                        drawingControlsEl.style.display = '';
                    }
                    if (undoBtn) {
                    ...
                    if (drawingControlsEl) {
                        drawingControlsEl.style.display = 'none';
                    }
                    if (undoBtn) {
            ...
            function updateHiddenFromPolygon() {
                ...
                if (google.maps.geometry && google.maps.geometry.encoding) {
                    var encoded = google.maps.geometry.encoding.encodePath(
                        path);
            ...
            function setButtonsState() {
                var hasPoly = !!polygon && polygon.getPath() &&
                              polygon.getPath().getLength() > 0;
            ...
            (
                function restoreState() {
                    var savedPath = (
                        hiddenEl && hiddenEl.value ? hiddenEl.value.trim() : ''
                    );
                    var hasSavedPath = savedPath.length > 0;
                    var hadPresetView = !!presetCenter || (
                        presetZoom !== null && presetZoom !== undefined
                    );

                    if (hasSavedPath && google.maps.geometry &&
                        google.maps.geometry.encoding) {
                        try {
                            var decoded = google.maps.geometry.encoding.decodePath(
                                savedPath);
            ...
            locateBtn.addEventListener('click', function() {
                ...
                geocoder.geocode({address: val}, function(results, status) {
                    if (status === 'OK' && results && results[0]) {
                        ...
                        alert((
                                  cfg.i18n && cfg.i18n.geocodeFailed
                              ) || 'Geocoding failed.');
                    }
                });
            });
        ...
        if (window.jQuery) {
            jQuery(document).on('gform_post_render', function(event, form_id) {
                initAll(
                    document.getElementById('gform_' + form_id) || document);
            });
        }
    }
)();
